...
data merge storage gen {l:{type:"structure",id:"mansion"}}
...
scoreboard players operation has_mansion gen = in_range gen
execute unless score in_range gen matches 1.. run return fail

data modify storage map:main dec append value {x:0,z:0,char:"m"}
