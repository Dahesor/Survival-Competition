...
data merge storage gen {l:{type:"structure",id:"gen:ancient_city"}}
...
scoreboard players operation has_ancient_city gen = in_range gen
execute unless score in_range gen matches 1.. run return fail

data modify storage map:main dec append value {x:0,z:0,char:"a"}
scoreboard players operation acx gen = x gen
scoreboard players operation acz gen = z gen
